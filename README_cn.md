...
基于地平线 **RDK X5** 边缘AI计算平台的溺水防控系统。YOLO11-Pose 逐帧提取 17 个 COCO 人体关键点，IoU 跟踪器维持跨帧人员 ID 一致，**GRU**（门控循环单元）分析 32 帧连续姿态序列，分类为四类行为：正常游泳、水中静止、溺水、已离开水域。

### 核心功能

- **边缘AI推理**：YOLO11-Pose 配合自定义 raw-head 解码器部署在 RDK X5 BPU，15–25 FPS
...
```bash
# 将模型文件放入项目根目录
cp yolo11m-pose_x5_rawhead.bin .
cp best_gru.bin .

# 设置环境变量
export POOL_POSE_MODEL=yolo11m-pose_x5_rawhead.bin
...
| 参数 | 值 |
|------|-----|
| 模型 | `yolo11m-pose_x5_rawhead.bin` |
| 输入形状 | [1, 3, 640, 640] |
